...
class ActivityController(private val activityService: ActivityService) {
    ...
    fun createActivity(@RequestBody input: ActivityInputModel, @Suppress("UNUSED_PARAMETER") user: User): ResponseEntity<*> =
        when (val result = activityService.createActivity(input.uid, input.tid, input.cid, input.type)) {
            is Either.Success -> ResponseEntity.status(HttpStatus.CREATED).body(ActivityOutputModel.fromDomain(result.value))
            is Either.Failure -> result.value.toProblemResponse()
        }
}
