...
class UserSessionController(
    ...
    fun login(
        ...
        when (val result = authService.login(loginInput.username, loginInput.password)) {
            is Either.Success -> {
    ...
    fun refresh(
        ...
        if (refreshTokenCookie == null) {
            return ResponseEntity
                ...
                .body(mapOf("error" to "Session expired. Please log in again."))
        }
    ...
    fun logout(
        ...
        val token =
            authHeader?.takeIf { it.startsWith("Bearer ") }?.substring(7)
