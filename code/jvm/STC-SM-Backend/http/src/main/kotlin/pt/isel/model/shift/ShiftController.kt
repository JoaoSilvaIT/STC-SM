...
class ShiftController(
    ...
    ): ResponseEntity<List<ShiftOutputModel>> = ResponseEntity.ok(shiftService.getAllShifts().map(ShiftOutputModel.Companion::fromDomain))
    ...
    fun startShift(
        ...
        when (val result = shiftService.startShift(id, user.id)) {
            is Either.Success -> {
                ResponseEntity
    ...
    fun endShift(
        ...
        when (val result = shiftService.endShift(id, user.id)) {
            is Either.Success -> {
                ResponseEntity
                    .status(HttpStatus.OK)
                    .body(ShiftOutputModel.fromDomain(result.value.shift))
    ...
    fun getShiftById(
        ...
        when (val result = shiftService.getShift(id)) {
            is Either.Success ->
                ResponseEntity
                    .status(HttpStatus.OK)
                    .body(ShiftOutputModel.fromDomain(result.value))
    ...
    fun getShiftByUserId(
        ...
        when (val result = shiftService.findShiftsByUser(id)) {
            is Either.Success -> {
                val responseBody =
                    result.value.map {
    ...
    fun getShiftByCabinetId(
        ...
        when (val result = shiftService.findShiftsByCabinet(id)) {
            is Either.Success -> {
                val responseBody =
                    result.value.map { shift ->
    ...
    fun isCabinetOccupied(
        @PathVariable cabinetId: Int,
        @Suppress("UNUSED_PARAMETER") user: User,
    ): ResponseEntity<Boolean> = ResponseEntity.ok(shiftService.hasActiveShift(cabinetId))
}
